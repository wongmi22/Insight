...
def get_predictions(month, date, players):
    ...
    cmd_all_players= 'SELECT DISTINCT(Player_Name) FROM NBA_player_data_MP;'
    df_players = pd.read_sql(cmd_date, con=conn) 
    df_all_players = pd.read_sql(cmd_all_players, con=conn) 
    player_all_list = list(df_all_players.Player_Name)
    ...
    conn.close()
    return predict_dict, averages_dict,team_dict,absent_dict,dummy_dict,accuracy_dict
...
def get_results_team(month,date,metric,players):
    ...
    prediction, avg_stats, teams, absent, dummy,acc = get_predictions(month,date,players)
    ...
    df_big_sorted_acc = df_big.sort(['Ametric'],ascending=False)
